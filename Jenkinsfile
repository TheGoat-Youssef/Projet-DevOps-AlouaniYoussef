pipeline {
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/TheGoat-Youssef/Projet-DevOps-AlouaniYoussef.git'
            }
        }
        ...
        stage('Notify Slack') {
            steps {
                slackSend(channel: env.SLACK_CHANNEL,tokenCredentialId: 'slack-token', message: "✅ Pipeline terminé avec succès pour Projet-DevOps-AlouaniYoussef")
            }
        }
    }
    ...
    post {
        failure {
           slackSend(
            ...
            message: "❌ Échec du pipeline pour Projet-DevOps-AlouaniYoussef"
        )
        }
    }
}
